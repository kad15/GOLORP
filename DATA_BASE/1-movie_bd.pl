:-discontiguous([movie/2,director/2,actor/3,actress/3]). % allow discontiguous predicates

% SWI Prolog ? :- style_check(-discontiguous).


test1(X) :- director(X, brian_de_palma).
test2(X) :- director(X, francis_ford_coppola); director(X,tim_burton).
test3(X) :- director(X, tim_burton).

test4(Actors):- acteur(american_beauty,Actors, _).
q1(A) :- director(M, brian_de_palma), acteur(M, A, _).
q2(A) :- (director(M, tim_burton);director(M, francis_ford_coppola)), acteur(M, A, _).
q3(A) :- (director(M, sofia_coppola), acteur(M, A, _)),(director(M2, sofia_coppola), acteur(M2, A, _)),M \= M2.q4(A,M) :- (director(M, tim_burton), acteur(M, A, _)),(director(M2, D), acteur(M2, A, _),  D\= francis_ford_coppola).

% true if A is an actor or actress in movie M
play_in(A,M) :- acteur(M, A,_).

%directed_by/2 true if Actor A has been directed by director D\
% the movie M has to be directed by D and A has to have played in movie M
directed_by(A,D):- director(M,D), acteur(M,A,_).

% director(M,D)  acteur
% true if there is a 3th different acteur who played with 
% A1 and A2 but in two different movies 
common_costar(A1, A2) :- 
		play_in(A1,M1),play_in(A3,M1),
		play_in(A2, M2), play_in(A3, M2), M1\=M2.

common_costar(A1, A2, A3, M1, M2) :- 
		play_in(A1,M1),play_in(A3,M1),A1\=A3,
		play_in(A2, M2), play_in(A3, M2), A3\=A2, M1\=M2.

trace1(M):- movie(M, 1999), write('movie = '),write(M), nl,director(M, sam_mendes). 
trace2(M):- movie(M, 1999), \+(director(M, sam_mendes)).

		
acteur(Movie, Actor, Role):- actor(Movie, Actor, Role); actress(Movie, Actor, Role).
